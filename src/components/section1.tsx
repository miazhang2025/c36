...
export default function IntroductionSection() {
  const [ref, isVisible] = useSectionFadeIn();

  // Elements to fade in
  const fadeElements = [
    <div className="flex-none !pb-[50px]" key="title">
      <h1 style={{ fontFamily: 'var(--font-kode-mono)' }} className="text-8xl text-[#FFFFFF] m-0 font-bold tracking-wider ">TL;DR</h1>
    </div>,
    <div style={{ fontFamily: 'var(--font-roboto-mono)' }} className=" flex flex-col gap-6 max-w-[600px] text-[#FFFFFF] tracking-wide text-[18px]" key="desc">
    ...
    >
      <div className="w-[90%] max-w-[1200px] h-auto !mb-[100px]">
        <div className="flex justify-between items-center gap-8 mb-20 ">
          {/* Stagger fade-in for title */}
          {fadeElements.map((el, idx) => (
            <div
              key={idx}
              className={`stagger-fadein${isVisible ? ' visible' : ''}`}
              style={{ transitionDelay: isVisible ? `${idx * 0.3}s` : '0s' }}
            >
              {el}
            </div>
          ))}
